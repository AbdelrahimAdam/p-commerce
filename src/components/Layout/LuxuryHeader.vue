<!-- src/components/Layout/LuxuryHeader.vue -->
<template>
      ...
      <div class="luxury-top-bar">
        ...
        <router-link :to="homeLink" class="luxury-logo-container">
          ...
          <div class="luxury-logo-text">
            ...
            <p class="logo-subtitle">{{ storeTagline }}</p>
          </div>
        </router-link>
            ...
            <div class="luxury-header-actions">
              ...
              <button v-if="!isAuthenticated" class="luxury-header-action" @click="router.push('/login')" :aria-label="languageStore.t('login')">
                <svg class="luxury-icon" viewBox="0 0 24 24" fill="none">
                  ...
                  <path d="M3 20C3 16.6863 5.68629 14 9 14H15C18.3137 14 21 16.6863 21 20V21H3V20Z" stroke="currentColor" stroke-width="1.5"/>
                </svg>
              </button>
        ...
        <div class="mobile-menu-content">
          <nav class="mobile-nav">
            <ul class="mobile-nav-list">
              <li class="mobile-nav-item"><router-link :to="homeLink" class="mobile-nav-link" @click="closeMobileMenu">{{ languageStore.t('home') }}</router-link></li>
              <li class="mobile-nav-item"><router-link to="/shop" class="mobile-nav-link" @click="closeMobileMenu">{{ languageStore.t('shop') }}</router-link></li>
              <li class="mobile-nav-item"><router-link to="/brands" class="mobile-nav-link" @click="closeMobileMenu">{{ languageStore.t('brands') }}</router-link></li>
              <li class="mobile-nav-item"><router-link to="/category/mens" class="mobile-nav-link" @click="closeMobileMenu">{{ languageStore.t('mens') }}</router-link></li>
              <li class="mobile-nav-item"><router-link to="/category/womens" class="mobile-nav-link" @click="closeMobileMenu">{{ languageStore.t('womens') }}</router-link></li>
              <li class="mobile-nav-item"><router-link to="/category/unisex" class="mobile-nav-link" @click="closeMobileMenu">{{ languageStore.t('unisex') }}</router-link></li>
              <li class="mobile-nav-item"><router-link to="/about" class="mobile-nav-link" @click="closeMobileMenu">{{ languageStore.t('about') }}</router-link></li>
              <li class="mobile-nav-item"><router-link to="/contact" class="mobile-nav-link" @click="closeMobileMenu">{{ languageStore.t('contact') }}</router-link></li>
            </ul>
          </nav>
          <div class="mobile-actions">
            <div class="mobile-action-btn" @click="closeMobileMenu"><LuxuryLanguageToggle /></div>
            <button class="mobile-action-btn" @click="handleMobileAction('search')">
              <svg class="mobile-action-icon" viewBox="0 0 24 24" fill="none"><path d="M21 21L15 15M17 10C17 13.866 13.866 17 10 17C6.13401 17 3 13.866 3 10C3 6.13401 6.13401 3 10 3C13.866 3 17 6.13401 17 10Z" stroke="currentColor" stroke-width="1.5"/></svg>
              <span>{{ languageStore.t('search') }}</span>
            </button>
            <template v-if="!isAuthenticated">
              <router-link to="/login" class="mobile-action-btn" @click="closeMobileMenu">
                <svg class="mobile-action-icon" viewBox="0 0 24 24" fill="none"><path d="M16 7C16 9.20914 14.2091 11 12 11C9.79086 11 8 9.20914 8 7C8 4.79086 9.79086 3 12 3C14.2091 3 16 4.79086 16 7Z" stroke="currentColor"/><path d="M3 20C3 16.6863 5.68629 14 9 14H15C18.3137 14 21 16.6863 21 20V21H3V20Z" stroke="currentColor"/></svg>
                <span>{{ languageStore.t('login') }}</span>
              </router-link>
              <router-link to="/register" class="mobile-action-btn" @click="closeMobileMenu">
                <svg class="mobile-action-icon" viewBox="0 0 24 24" fill="none"><path d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" stroke="currentColor"/></svg>
            ...
            <router-link to="/returns" class="mobile-action-btn" @click="closeMobileMenu">
              <svg class="mobile-action-icon" viewBox="0 0 24 24" fill="none"><path d="M16 15v-1a4 4 0 00-8 0v1m-4 0a4 4 0 014-4h8a4 4 0 014 4v4a4 4 0 01-4 4H8a4 4 0 01-4-4v-4z" stroke="currentColor"/><path d="M12 5v4" stroke="currentColor" stroke-width="2"/></svg>
              <span>{{ languageStore.t('returns') }}</span>
            </router-link>
            <router-link v-if="isAdmin" to="/admin" class="mobile-action-btn" @click="closeMobileMenu">
              <svg class="mobile-action-icon" viewBox="0 0 24 24" fill="none"><path d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" stroke="currentColor"/><path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" stroke="currentColor"/></svg>
              <span>{{ languageStore.t('adminPanel') }}</span>
            </router-link>
            <button v-if="isAuthenticated" @click="handleLogout" class="mobile-action-btn logout-btn">
              <svg class="mobile-action-icon" viewBox="0 0 24 24" fill="none"><path d="M17 16L21 12M21 12L17 8M21 12H7M13 16C13 17.6569 11.6569 19 10 19H6C4.34315 19 3 17.6569 3 16V8C3 6.34315 4.34315 5 6 5H10C11.6569 5 13 6.34315 13 8" stroke="currentColor"/></svg>
    ...
    <div v-if="userMenuOpen && isAuthenticated" class="luxury-dropdown user-dropdown" @click.stop>
      <div class="dropdown-content">
        <div class="dropdown-user">
          <div class="user-avatar"><span class="avatar-initials">{{ userInitials }}</span></div>
          <div class="user-info">
        ...
        <div class="dropdown-menu">
          <router-link to="/account" class="dropdown-item" @click="userMenuOpen = false">
            <svg class="dropdown-icon" viewBox="0 0 24 24" fill="none"><path d="M12 12C14.2091 12 16 10.2091 16 8C16 5.79086 14.2091 4 12 4C9.79086 4 8 5.79086 8 8C8 10.2091 9.79086 12 12 12Z" stroke="currentColor"/><path d="M6 20C6 16.6863 8.68629 14 12 14C15.3137 14 18 16.6863 18 20V21H6V20Z" stroke="currentColor"/></svg>
            <span>{{ languageStore.t('profile') }}</span>
          </router-link>
          <router-link to="/orders" class="dropdown-item" @click="userMenuOpen = false">
            <svg class="dropdown-icon" viewBox="0 0 24 24" fill="none"><path d="M9 5H7C5.89543 5 5 5.89543 5 7V19C5 20.1046 5.89543 21 7 21H17C18.1046 21 19 20.1046 19 19V7C19 5.89543 18.1046 5 17 5H15M9 5C9 6.10457 9.89543 7 11 7H13C14.1046 7 15 6.10457 15 5M9 5C9 3.89543 9.89543 3 11 3H13C14.1046 3 15 3.89543 15 5M12 12H15M12 16H15M9 12H9.01M9 16H9.01" stroke="currentColor"/></svg>
            <span>{{ languageStore.t('orders') }}</span>
          </router-link>
          <router-link to="/returns" class="dropdown-item" @click="userMenuOpen = false">
            <svg class="dropdown-icon" viewBox="0 0 24 24" fill="none"><path d="M16 15v-1a4 4 0 00-8 0v1m-4 0a4 4 0 014-4h8a4 4 0 014 4v4a4 4 0 01-4 4H8a4 4 0 01-4-4v-4z" stroke="currentColor"/><path d="M12 5v4" stroke="currentColor" stroke-width="2"/></svg>
            <span>{{ languageStore.t('returns') }}</span>
          </router-link>
          <router-link to="/wishlist" class="dropdown-item" @click="userMenuOpen = false">
            <svg class="dropdown-icon" viewBox="0 0 24 24" fill="none"><path d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" stroke="currentColor"/></svg>
            <span>{{ languageStore.t('wishlist') }}</span>
          </router-link>
          <router-link v-if="isAdmin" to="/admin" class="dropdown-item" @click="userMenuOpen = false">
            <svg class="dropdown-icon" viewBox="0 0 24 24" fill="none"><path d="M10.325 4.317C10.751 2.561 13.249 2.561 13.675 4.317C13.861 5.151 14.687 5.657 15.514 5.421C17.227 4.926 18.675 6.374 18.18 8.087C17.944 8.914 18.45 9.74 19.284 9.926C21.04 10.352 21.04 12.85 19.284 13.276C18.45 13.462 17.944 14.288 18.18 15.115C18.675 16.828 17.227 18.276 15.514 17.781C14.687 17.545 13.861 18.051 13.675 18.885C13.249 20.641 10.751 20.641 10.325 18.885C10.139 18.051 9.313 17.545 8.486 17.781C6.773 18.276 5.325 16.828 5.82 15.115C6.056 14.288 5.55 13.462 4.716 13.276C2.96 12.85 2.96 10.352 4.716 9.926C5.55 9.74 6.056 8.914 5.82 8.087C5.325 6.374 6.773 4.926 8.486 5.421C9.313 5.657 10.139 5.151 10.325 4.317Z" stroke="currentColor"/><path d="M15 12C15 13.6569 13.6569 15 12 15C10.3431 15 9 13.6569 9 12C9 10.3431 10.3431 9 12 9C13.6569 9 15 10.3431 15 12Z" stroke="currentColor"/></svg>
            <span>{{ languageStore.t('adminPanel') }}</span>
          </router-link>
          <button @click="handleLogout" class="dropdown-item logout-btn">
            <svg class="dropdown-icon" viewBox="0 0 24 24" fill="none"><path d="M17 16L21 12M21 12L17 8M21 12H7M13 16C13 17.6569 11.6569 19 10 19H6C4.34315 19 3 17.6569 3 16V8C3 6.34315 4.34315 5 6 5H10C11.6569 5 13 6.34315 13 8" stroke="currentColor"/></svg>
            <span>{{ languageStore.t('logout') }}</span>
          </button>
        </div>
      </div>
      ...
      <div class="search-modal" @click.stop>
        <div class="search-input-container">
          ...
          <button class="search-close" @click="searchOpen = false">
            <svg class="search-icon" viewBox="0 0 24 24" fill="none"><path d="M6 18L18 6M6 6L18 18" stroke="currentColor" stroke-width="1.5"/></svg>
          </button>
        </div>
      </div>
    </div>
...
const handleClickOutside = (event: MouseEvent) => {
  ...
  if (mobileMenuOpen.value && target.closest('.luxury-mobile-overlay')) closeMobileMenu()
  if (userMenuOpen.value && !target.closest('.user-dropdown') && !target.closest('.luxury-header-action[aria-label="account"]')) {
    userMenuOpen.value = false
  }
}
...
const updateDirection = () => {
  ...
  if (isRTL.value) document.body.classList.add('rtl')
  else document.body.classList.remove('rtl')
}

watch(() => languageStore.currentLanguage, () => updateDirection())
...
<style scoped>
/* All styles remain exactly as in your original file */
.luxury-header {
  background: linear-gradient(180deg, rgba(10, 10, 10, 0.98) 0%, rgba(26, 18, 11, 0.95) 100%);
...
@keyframes logo-glow {
  ...
  50% { box-shadow: 0 0 50px rgba(212, 175, 55, 0.5), 0 0 100px rgba(212, 175, 55, 0.2); }
}
