...
<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <!-- Not Authorized -->
    <div v-else-if="!isAuthorized" class="flex items-center justify-center min-h-screen">
    ...
    <template v-else>
      ...
      <div class="p-8">
        ...
        <!-- Orders Table (keep the same as original) -->
        <div class="bg-white rounded-xl shadow-luxury overflow-hidden">
          ...
          <!-- Orders Table (keep the rest of your template) -->
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                ...
                >
                  <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <div class="text-sm text-gray-500">{{ order.items?.length || 0 }} {{ t('items') }}</div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm font-medium text-gray-900">{{ order.customer?.name || t('Guest') }}</div>
                    <div class="text-sm text-gray-500">{{ order.customer?.email || order.userEmail || t('No email') }}</div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ formatDate(order.createdAt) }}</td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm font-bold text-gray-900">EGP {{ formatCurrency(order.total) }}</div>
                    <div class="text-sm text-gray-500">{{ getPaymentMethodText(order.paymentMethod) }}</div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div class="flex items-center gap-3">
                      <button @click="viewOrder(order)" class="text-primary-600 hover:text-primary-700" :title="t('View Details')">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                        </svg>
                      </button>
                      <button @click="updateStatus(order)" class="text-gray-600 hover:text-primary-600" :title="t('Update Status')">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                        </svg>
                      </button>
                      <button @click="printInvoice(order)" class="text-gray-600 hover:text-primary-600" :title="t('Print Invoice')">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div v-if="!ordersStore.loading && !ordersStore.error && filteredOrders.length === 0" class="p-12 text-center">
            ...
            <h3 class="text-xl font-bold text-gray-900 mb-2">{{ t('No orders found') }}</h3>
            <p class="text-gray-600">{{ hasActiveFilters ? t('Try adjusting your filters') : t('No orders have been placed yet') }}</p>
          </div>
          ...
          <div v-if="!ordersStore.loading && !ordersStore.error && filteredOrders.length > 0" class="px-6 py-4 border-t border-gray-200">
            <div class="flex items-center justify-between">
              <div class="text-sm text-gray-700">
                {{ t('Showing') }} <span class="font-medium">{{ startIndex + 1 }}</span>
                {{ t('to') }} <span class="font-medium">{{ endIndex }}</span>
                {{ t('of') }} <span class="font-medium">{{ filteredOrders.length }}</span>
                {{ t('orders') }}
              </div>
              <div class="flex items-center gap-2">
                <button @click="prevPage" :disabled="currentPage === 1" class="px-3 py-1 border border-gray-300 rounded text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50">
                  {{ t('Previous') }}
                </button>
                <span class="px-3 py-1 text-sm text-gray-700">{{ currentPage }} / {{ totalPages }}</span>
                <button @click="nextPage" :disabled="currentPage === totalPages" class="px-3 py-1 border border-gray-300 rounded text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50">
                  {{ t('Next') }}
                </button>
              </div>
            </div>
...
const checkAuth = async () => {
  ...
  try {
    ...
    if (!authenticated || !isAdmin) {
      console.log('Not authorized for admin orders page')
...
const confirmStatusUpdate = async () => {
  if (!orderToUpdate.value || !isAuthorized.value) return
